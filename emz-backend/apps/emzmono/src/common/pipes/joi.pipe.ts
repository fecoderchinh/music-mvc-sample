...
import {
    ...
    Optional,
    PipeTransform,
    Scope, UnprocessableEntityException,
} from '@nestjs/common';
...
import {Constructor} from "nestjs-joi/internal/defs";
...
export class JoiPipe implements PipeTransform {
    ...
    private static validate<T>(
        ...
        const { error, value } = Joi.compile(schema).prefs({ errors: { label: 'key' } }).validate(
            payload,
